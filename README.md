...
当前 Docker 版本内置管理面板，访问 `http://服务器地址:8877/` 即可使用。

- 首次启动未设置 `ADMIN_PASSWORD` 时，会随机生成管理员密码并写入容器日志，可用 `docker compose logs freebuff2api` 查看。
- 面板支持一键创建 FreeBuff CLI 授权链接，并自动轮询授权结果加入账号池。
- 每个账号可独立配置 `http://`、`https://` 或 `socks5://` 代理地址。
- 账号池采用活跃 session 亲和策略，只有当前账号明确限流、失效或冷却时才切换，减少重复 session 初始化造成的延迟和额度消耗。
- 管理会话使用 `HttpOnly`、`SameSite=Strict` Cookie。HTTPS 部署时设置 `COOKIE_SECURE=true`。
- WebUI 账号数据持久化在 `./data/accounts.json`，文件不会进入 Docker 镜像。
...
```dotenv
ADMIN_PASSWORD=替换为高强度管理密码
COOKIE_SECURE=true
FREEBUFF_API_KEY=替换为API调用密钥
```
