...
const accountProxyUrls = new Map();
const proxyDispatchers = new Map();

function syncAccountProxies(accounts) {
  for (const account of accounts) {
    if (account.proxyUrl) accountProxyUrls.set(account.token, account.proxyUrl);
    else accountProxyUrls.delete(account.token);
  }
}

function accountFetch(token, url, init = {}) {
  const proxyUrl = accountProxyUrls.get(token);
...
function parseAccounts(env) {
  if (env?.FREEBUFF_ACCOUNTS) {
    try {
      const configured = JSON.parse(env.FREEBUFF_ACCOUNTS);
      if (Array.isArray(configured)) {
        const accounts = configured
          .filter((account) => account && typeof account.token === "string" && account.token.trim())
          .map((account) => ({
            token: account.token.trim(),
            proxyUrl: typeof account.proxyUrl === "string" ? account.proxyUrl.trim() : "",
            id: typeof account.id === "string" ? account.id : "",
          }));
        if (accounts.length) {
          syncAccountProxies(accounts);
          return accounts;
        }
      }
    } catch (error) {
      console.error("[accounts] invalid FREEBUFF_ACCOUNTS:", error.message);
    }
  }
...
async function up(method, path, token, body, extraHeaders = {}, timeoutMs = UPSTREAM_TIMEOUT_MS) {
  ...
  const resp = await accountFetch(token, CODEBUFF_API + path, {
    method,
    headers,
...
async function fetchStreamWithQuotaGuard(url, init, token, sessionModel) {
  ...
  const request = accountFetch(token, url, { ...init, signal: controller.signal });
  let probeTimer = null;
      ...
      for (let attempt = 0; attempt < 2; attempt++) {
        ...
        try {
          resp = isStream
            ...
            : await accountFetch(token, CODEBUFF_API + "/api/v1/chat/completions", {
                ...chatInit,
